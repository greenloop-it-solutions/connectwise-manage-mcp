...
- **Time entry correction tools.** Entries could be created but never fixed. Three tools close the loop: `cw_update_time_entry` (`PATCH /time/entries/{id}`, JSON Patch — change `chargeToId`/`chargeToType`, `workType/id`, `workRole/id`, `billableOption`, notes, `timeStart`/`timeEnd`, `actualHours`), `cw_delete_time_entry` (`DELETE /time/entries/{id}`, guarded — refuses entries whose status is not `Open`), and `cw_search_charge_codes` (`GET /time/chargeCodes`, conditions/paging) for resolving a charge code name such as "Meeting" to the numeric ID that `chargeToId` requires.
