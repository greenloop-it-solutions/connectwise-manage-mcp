...
This is a [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server that gives Claude (or any MCP-compatible AI) 77 tools covering the daily operations ConnectWise Manage shops depend on. Works with both **cloud-hosted and self-hosted** CWM instances — just point it at your server.
...
- `cw_get_ticket_tasks` — Get all checklist tasks on a ticket
- `cw_get_ticket_task` — Get a single checklist task by task ID
- `cw_update_ticket_task` — Update a checklist task (JSON Patch — e.g. mark complete via `closedFlag`)
...
- `cw_get_project_ticket_tasks` — Get all checklist tasks on a project ticket
- `cw_get_project_ticket_task` — Get a single checklist task by task ID
- `cw_update_project_ticket_task` — Update a checklist task (JSON Patch — e.g. mark complete via `closedFlag`)
