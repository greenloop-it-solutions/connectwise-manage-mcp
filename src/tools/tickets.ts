...
export function registerTicketTools(server: McpServer, client: CwManageClient) {
  ...
  server.tool(
    "cw_get_ticket_tasks",
    "Get all checklist tasks on a service ticket (the task list/sub-items inside the ticket, not the ticket itself).",
    {
      id: z.number().describe("Service ticket ID"),
      page: z.number().optional().describe("Page number (default: 1)"),
      pageSize: z.number().optional().describe("Results per page (default: 25, max: 1000)"),
    },
    async ({ id, page, pageSize }) => {
  ...
  server.tool(
    "cw_get_ticket_task",
    "Get a single checklist task on a service ticket by task ID.",
    {
      id: z.number().describe("Service ticket ID"),
      taskId: z.number().describe("Task ID"),
    },
    async ({ id, taskId }) => {
      const result = await client.get(`/service/tickets/${id}/tasks/${taskId}`);
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    },
  );
  ...
  server.tool(
    ...
    "Update a checklist task on a service ticket using JSON Patch operations (e.g. set closedFlag to mark complete, edit notes, or change sequence/priority).",
    {
      id: z.number().describe("Service ticket ID"),
      taskId: z.number().describe("Task ID"),
      operations: z
    ...
    async ({ id, taskId, operations }) => {
      const result = await client.patch(`/service/tickets/${id}/tasks/${taskId}`, operations);
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    },
  );
}
